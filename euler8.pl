...
#$products = $num=~ /(\d)(\d){\d}(\d)(\d)/{$1*$2*$3*$4*$5}/rge;
my @products;
my $prev;
...
while ($num=~/(\d)(\d)(\d)(\d)(\d)/gi) {
        #print $1*$2*$3*$4*$5."\n";
    #push @products, $1*$2*$3*$4*$5;          # Put the product in our array.
...
#my @sortedproducts = sort numerically @products;

#print "Greatest value is $sortedproducts[1].\n";
